  ...
  if (!hasContent) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 16, color: "var(--text-muted)" }}>
        <div style={{ width: 56, height: 56, borderRadius: 12, background: "var(--bg-elevated)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {icon}
        </div>
    ...
    <div style={{ height: "100%", overflowY: "auto", background: "var(--bg-base)" }}>
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "24px 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 24, padding: "16px 20px", background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 12 }}>
          ...
          <div style={{ marginLeft: "auto", padding: "6px 14px", background: "#1c1207", border: "1px solid #92400e", borderRadius: 8, fontSize: 12, color: "#fbbf24", fontFamily: "var(--font-code)", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 6 }}>
            <Construction size={13} /> En construcción
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {/* Teoría */}
          {teoria.length > 0 && (
            <ContentCard title="Teoría" color="#38bdf8" icon={<BookOpen size={13} />}>
              {teoria.map((t, i) => (
          ...
          {/* Laboratorio */}
          {lab.length > 0 && (
            <ContentCard title="Laboratorio" color="#4ade80" icon={<FlaskConical size={13} />}>
              {lab.map((t, i) => (
                <ItemRow key={i} label={t.label} done={false} />
              ))}
            </ContentCard>
        ...
        {viz.length > 0 && (
          <div style={{ marginTop: 16, background: "var(--bg-surface)", border: "1px dashed #7c3aed", borderRadius: 12, padding: "16px 20px" }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#a78bfa", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
              <Palette size={13} /> Visualizadores Interactivos / Excalidraw — Planeados
            </div>
...
function ContentCard({ title, color, icon, children }: { title: string; color: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
