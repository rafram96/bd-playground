...
export default function S8Exam() {
  return (
    <GuideLayout sections={SECTIONS}>
          ...
          <DefList items={[
            [<Code key="vector">to_tsvector</Code>, <>texto → documento procesado (lexemas normalizados + posiciones). Es el Bag of Words.</>],
            [<Code key="query">to_tsquery</Code>, <>texto → consulta de lexemas con <Code>&amp;</Code> (AND), <Code>|</Code> (OR), <Code>!</Code> (NOT).</>],
            [<Code key="match">@@</Code>, <>operador de coincidencia: <Code>tsvector @@ tsquery</Code> → ¿el documento satisface la consulta?</>],
            [<Code key="gin">GIN</Code>, <>Generalized Inverted Index: <Bold>es el índice invertido</Bold> de PostgreSQL. Lo que hace que escale.</>],
            [<Code key="rank">ts_rank</Code>, <>calcula el score de relevancia para ordenar (el ranking).</>],
          ]} />
